    ...
    def draw_polygons(self):
        ...
        for poly in self.polygons:
            ...
            for p in poly:                
                x,y = p
                p_id = "%s,%s" % (x,y)
                key = gd.polygon_lookup[p_id] if p_id in gd.polygon_lookup else None
                if key in gd.selected_polygon_dict and gd.selected_polygon_dict[key]:
                    boarder = 5
...
def message_display(x, y):
    #font to print name to screen
    myfont = pygame.font.SysFont("arial", 20)
    label = myfont.render(key, 40, (0,0,0))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for key, polygons in gd.adjusted_poly_dict.items():
                    #the country is unselected since the point is not found as yet
                    #gd.selected_polygon_dict[key] = False
                    gd.selected_polygon_dict.clear() 
